...
$(document).ready(function() {
    $.get("ajax/principal.php", {accion:"cboAulas"},
        function(data) {
...
$(document).ready(function() {
    $.get("ajax/principal.php", {accion:"cboDocentes"},
        function(data) {
...
$("#cboperiodo").select2();
$(document).ready(function(){
    $.get("ajax/principal.php",{accion:"cboCursos"},
        function(datoscursos){
            ...
            $.get("ajax/principal.php",{accion:"cboPeriodos"},
            function(data){
                ...
                $.get("ajax/principal.php",{accion:"cboCursosPeriodo",vercurricular:vercurricular},
                function(dtcursos){
                ...
                {   
                    ...
                    $("#tabla-carga").html("");
                    $("#tabla-carga").html('<center><img style="height:80px;" src="assets/img/cargando.gif"/></center>').fadeIn();
                    // --------------------------
                    ...
                    correspondecia(curso,vercurricular);
						...
						$.get("ajax/principal.php", {accion:"MostrarHorariosTabla",
                            curso: curso,
                            ciclo: ciclo,
                            estado: 1
                        },
                        function(data) {
                            ...
                            $.get("ajax/principal.php",{accion:"versionCurricular"},
...
function CrearTablaPrincipal(datosJson, cboaulas, cbodocentes) {
    $("#tabla-carga").attr("class", "table-responsive-principal");
    ...
    tabla.setAttribute("class", "tabla-principal border rounded");
    ...
    for (i = 0; i < cantidad; i++) {
        ...
        $("#tabla-principal").append(
            ...
            "<td><input id='txttope" + id + "' type='text' disabled class='i txtform' spellcheck='false' autocomplete='off' value='" + datosJSON[i]['tope'] + "'></td>" +
    ...
    $("#tabla-principal").append(
        ...
        "<td><input id='txttope' type='text' class='form-control-n txtform' spellcheck='false' autocomplete='off' maxlength='3'></td>" +
...
$("#list-aulas-disponibles-t").click(function(e){
    e.preventDefault();
    ...
    estadoAula=1;
    buscarAulasVacia(dia,hora,cboPeriodo,estadoAula);
});

$("#list-aulas-disponibles-c").click(function(e){
    e.preventDefault();
...
$("#list-aulas-disponibles-l").click(function(e){
    e.preventDefault();
...
$("#list-aulas-t").click(function(e){
    $('#modal-aulas-disponibles').modal('show');
    mostrarAulas(1);
});

$("#list-aulas-c").click(function(e){
    $('#modal-aulas-disponibles').modal('show');
    mostrarAulas(2);
});

$("#list-aulas-l").click(function(e){
    $('#modal-aulas-disponibles').modal('show');
    mostrarAulas(3);
});

aulasLLenas=new Array();
function buscarAulasVacia(dia,hora,periodo,estadoAula){
    ...
    $.get("views/Anexos/principal.php",{accion:"cargaHorariaPorDia",dia:dia,periodo:periodo,estadoAula:estadoAula},
        function(data){
            ...
            for(p=0;p<canAulas;p++)
            { 
                if(cboaulas[p]["aula"]==aulaactual)
                {

                    $('#tabla-aulas-disponibles').append("<tr class='tr-principal'><td>"+cboaulas[p]["aula"]+"</td>"+
                                                        "<td>"+cboaulas[p]["capacidad"]+" "+cboaulas[p]["tipSilla"]+"</td>"+
                                                        "<td>"+cboaulas[p]["tipEntrada"]+"</td>"+
                                                        "<td>"+cboaulas[p]["pizarra"]+"</td>"+
                                                        "<td>"+mostrarCondicion(cboaulas[p]["taburete"])+"</td>"+
                                                        "<td>"+mostrarCondicion(cboaulas[p]["pcDocente"])+"</td>"+
                                                        "<td>Actual</td>"+"</tr>");
                }
            }
            ...
            {   
                ocupado=0;
                if(cboaulas[u]["estado"]==estadoAula)
                {
                    ...
                    if(ocupado==0)
                    {
                        $('#tabla-aulas-disponibles').append("<tr><td>"+cboaulas[u]["aula"]+"</td>"+
                                                            "<td>"+cboaulas[u]["capacidad"]+" "+cboaulas[u]["tipSilla"]+"</td>"+
...
function mostrarAulas(estado){
    ...
    { 
        ...
        {
            $('#tabla-aulas-disponibles').append("<tr class='tr-principal'><td>"+cboaulas[p]["aula"]+"</td>"+
                                                "<td>"+cboaulas[p]["capacidad"]+" "+cboaulas[p]["tipSilla"]+"s</td>"+
                                                "<td>"+cboaulas[p]["tipEntrada"]+"</td>"+
                                                "<td>"+cboaulas[p]["pizarra"]+"</td>"+
                                                "<td>"+mostrarCondicion(cboaulas[p]["taburete"])+"</td>"+
                                                "<td>"+mostrarCondicion(cboaulas[p]["pcDocente"])+"</td>"+
        ...
        {   
            ...
            {
                $('#tabla-aulas-disponibles').append("<tr><td>"+cboaulas[u]["aula"]+"</td>"+
                                                    "<td>"+cboaulas[u]["capacidad"]+" "+cboaulas[u]["tipSilla"]+"s</td>"+
                                                    "<td>"+cboaulas[u]["tipEntrada"]+"</td>"+
                                                    "<td>"+cboaulas[u]["pizarra"]+"</td>"+
                                                    "<td>"+mostrarCondicion(cboaulas[u]["taburete"])+"</td>"+
                                                    "<td>"+mostrarCondicion(cboaulas[u]["pcDocente"])+"</td>"+
...
function compararHoras(hora,horacarga){
    ...
    for(inicial=(hicarga+1);inicial<hfcarga;inicial++)
    {
...
/*
    if (hfinal>hicarga && hinicial<hfcarga)
        ocupado="SI";
    else
        ocupado="NO";
*/
...
function correspondecia(curso,vercurricular){
    ...
    $.get("ajax/principal.php",{accion:"correspondencia",codCurso:curso,verCurricular:vercurricular},
...
function editar(indice) {
    ...
    //****************************
    $("#txttope" + indice).prop("disabled", false);
    $("#txttope" + indice).addClass('form-control-n');
    $("#txttope" + indice).removeClass('i');
    //****************************
...
function salir(indice) {
    ...
    // //*******************************
    $("#txttope" + indice).prop("disabled", true);
    $("#txttope" + indice).addClass('i');
    $("#txttope" + indice).removeClass('form-control-n');
    //*******************************
...
function guardar() {
    ...
    datos.append("txttope", $("#txttp").val());
...
function actualizar(indice) {
    ...
    datos.append("txttope", $("#txttope" + indice).val());
